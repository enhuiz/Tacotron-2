...
def run_synthesis(args, checkpoint_path, output_dir, hparams):
    ...
    if args.model == "Tacotron-2":
        ...
        mel_files = sorted(
            [
                os.path.join(args.mels_dir, f)
                for f in os.listdir(args.mels_dir)
                if f.split(".")[-1] == "npy"
            ]
        )
        speaker_ids = (
            ...
            else args.speaker_id.replace(" ", "").split(",")
        )
    ...
    speaker_ids = (
        ...
        if speaker_ids is None
            ...
            speaker_ids[i : i + hparams.wavenet_synthesis_batch_size]
            for i in range(0, len(speaker_ids), hparams.wavenet_synthesis_batch_size)
        ]
    )
    texts = (
        None
    ...
    with open(os.path.join(wav_dir, "map.txt"), "w") as file:
        for i, mel_batch in enumerate(tqdm(mel_files)):
            ...
            basenames = [
                os.path.basename(mel_file).replace(".npy", "") for mel_file in mel_batch
            ]
            speaker_id_batch = None if speaker_ids is None else speaker_ids[i]
            audio_files = synth.synthesize(
                mel_spectros, speaker_id_batch, basenames, wav_dir, log_dir
            )

            speaker_logs = (
                ["<no_g>"] * len(mel_batch)
                if speaker_id_batch is None
                else speaker_id_batch
            )
            ...
            for j, mel_file in enumerate(mel_batch):
                if texts is None:
                    ...
                    file.write(
                        "{}|{}|{}\n".format(
                            texts[i][j], mel_file, audio_files[j], speaker_logs[j]
                        )
                    )
